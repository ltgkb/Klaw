...
本地 BGE-M3 已按用户决定停用，Compose 默认不再启动 TEI。当前 Kaiweb 账号只暴露 `glm-4.5` 和 `glm-5.2` 聊天模型，`/v1/embeddings` 返回 400，不能充当 Embedding API。剩余条件是配置一个真实支持 `/v1/embeddings`、且返回 1024 维向量的 API 地址、Key 和模型名；健康检查会真实请求并校验维度。
