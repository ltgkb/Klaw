...
- 默认使用已配置的 Embedding API，只需下载约 500MB 的多语言 reranker。只有启用 `local-embedding` profile 时才下载约 2GB 的 BGE-M3。
...
EMBEDDING_API_BASE_URL=<OpenAI兼容地址，通常以/v1结尾>
EMBEDDING_API_KEY=<API密钥>
EMBEDDING_API_MODEL=<返回1024维向量的模型>
...
2026-07-20 已在 8GB x86_64 服务器上完成：

1. 多语言 `cross-encoder/mmarco-mMiniLMv2-L12-H384-v1` 为 `healthy`，无重启、无 OOM。
2. 真实中文 `/rerank` 中，知识库相关中文得分 `0.9819`、英文得分 `0.5227`、天气文本得分 `0.0003`，排序正确。
3. OpenClaw `/readyz`、`/v1/models` 和 Hermes `/health` 已通过。
4. PostgreSQL 保持原容器、原数据卷和 `4b2e9a1c7d33` schema，覆盖部署未重建数据库。
5. 硅基流动 `BAAI/bge-m3` Embedding API 已真实返回 1024 维向量，Key 通过应用加密后存入 `system_settings`。
6. 公网 `/api/v1/health` 返回 `healthy`，Embedding 为 `ok: embedding API`。
7. OpenClaw 默认聊天链路为 `Klaw -> OpenClaw -> Kaiweb glm-4.5`；Kaiweb 直连仅作 OpenClaw 故障 fallback。

本地 BGE-M3 已按用户决定停用，Compose 默认不再启动 TEI。Embedding 改走硅基流动 API；健康检查会真实请求 `/embeddings` 并校验维度，不能用聊天模型冒充 Embedding 模型。

旧设备测试还确认：`BAAI/bge-reranker-v2-m3` 会引发整套环境 OOM，`BAAI/bge-reranker-base` 在 4GB 容器限制内加载失败，CPU TEI 也不能强制使用 float16。不要切回这些默认值，除非同步提高 Docker 和容器内存。
...
```bash
docker stats --no-stream claw-reranker claw-es claw-openclaw claw-hermes
docker inspect claw-reranker --format '{{.State.Health.Status}} restarts={{.RestartCount}} oom={{.State.OOMKilled}}'
```

通过标准：`claw-reranker` 为 `healthy`，`RestartCount=0`，`OOMKilled=false`。首次下载慢时继续等，不要因健康状态仍是 `starting` 就删除持久卷。仅需本地 Embedding 时才执行 `docker compose --profile local-embedding up -d tei`。
...
通过标准：在设置页确认 Embedding source 为 `api`，上传并解析文档可生成 1024 维向量；rerank 返回 3 项、包含 `index` 和 `score`，知识库相关文本排在天气文本之前。
...
通过标准：返回 `status: healthy`；基础设施、OpenClaw、Hermes、reranker 全部为 `ok`，`embedding` 显示 `ok: embedding API`。健康检查会真实调用 `/embeddings` 并校验返回 1024 维，不会把仅填写地址的无效配置算作健康。
...
- 旧设备上的模型容器已停止；最终运行状态以服务器和远端分支为准。
