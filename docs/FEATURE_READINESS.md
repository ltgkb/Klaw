...
8. 新增 GitHub Actions CI，所有分支 push 与面向 main 的 PR 均会并行执行后端测试、前端 lint/build 与 Compose 配置校验，并限制最小只读权限。
...
- P0：无代码层遗留；生产部署必须设置强 JWT/加密密钥，并至少配置一个真实 embedding 与 LLM provider；启用本地工具时再提供 OpenClaw/Hermes。
- P1：真实 provider/工具/推送成功链路未验证；前端对话流式与多数页面错误态；画布工具节点；真实长耗时暂停/取消；浏览器 E2E 自动化。
- P2：OCR/多模态、Redis TTL 短期记忆、重排服务、文件版本、推送重试/告警、多实例调度锁、可观测/评测、K8s/压测。
