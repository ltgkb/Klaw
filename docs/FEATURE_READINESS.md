...
- 后端基线：承接分支后 `uv run pytest -q` 为 **220 passed**；本轮新增通知渠道、工具节点、调用边界、Office parser 与认证安全测试，最终全量回归 **240 passed**。
...
- 真实 API：注册/登录、TXT/DOCX/XLSX/PPTX→MinIO→解析→hash embedding→ES 检索引用、OpenClaw `web_fetch`、画布 tool 节点、加密通知渠道/owner 拦截、APScheduler 实际触发及暂停后重启保持均通过。
...
| TXT/MD/HTML/JSON/DOCX/XLSX/PPTX/EPUB | KB 详情上传 | 可用 | MinIO + DeepDoc 类型路由；修复 PPTX 返回值契约 | 生成文件 parser tests；真实 TXT/DOCX/XLSX/PPTX 全链路并检索 marker | 部分可用 | MD/HTML/JSON/EPUB 缺逐格式 MinIO->ES E2E（P1） |
...
| 推送渠道与失败重试 | 设置、notify 节点 | 创建/原地编辑/测试；节点选择渠道；旧配置可清除 | owner-scoped `channel_ids` 解密；PUT 保持 ID；SSRF/DNS pin | 240 tests；真实密文保留/轮换且 flow 引用不变；跨 owner 拦截 | 部分可用 | 无真实外部推送凭据；无持久重试（P1/P2） |
...
| 测试/lint/build/CI | Makefile、GitHub Actions | N/A | pytest/oxlint/tsc/Vite/Compose jobs；health tests 隔离本机服务 | 240 tests；lint/build/config/YAML 通过 | 部分可用 | OAuth 缺 `workflow` scope，CI 尚未推送运行；bundle 636 kB；无浏览器 CI（P1） |
